"""Scraper for Pestana Leilões (pestanaleiloes.com.br)

Traditional auctioneer for vehicles, real estate, and materials.
Listings accessible via /search page with dynamic JS filters and an
internal API at /sgl/v1/leiloes.
"""
from .base_scraper import BaseScraper
import config


class PestanaLeiloesScraper(BaseScraper):

    # Pestana uses a SPA with JS rendering; Selenium is the primary strategy.
    ...
    LINK_PATTERNS = ["/leilao/", "/lote/", "/search/"]

    def __init__(self):
        super().__init__("pestana_leiloes", "https://www.pestanaleiloes.com.br")

    def scrape(self):
        self.logger.info("Scraping Pestana Leiloes...")
        try:
            self._scrape_listings()
        except Exception as e:
            self.logger.error(f"Error scraping Pestana Leiloes: {e}")

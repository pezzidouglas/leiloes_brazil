"""Scraper for Leiloes Judiciais (leiloesjudiciais.com.br)

Well-structured site with category pages and pagination.
URLs: /imoveis, /imoveis/casas, /veiculos, /veiculos/motos?pagina=N, /diversos
"""
...
class LeiloesJudiciaisScraper(BaseScraper):
    ...
    LINK_PATTERNS = ["/lote/", "/imovel/", "/veiculo/", "/lotes/", "/busca/"]

    def __init__(self):
        super().__init__("leiloes_judiciais", "https://www.leiloesjudiciais.com.br")

    def scrape(self):
        self.logger.info("Scraping Leiloes Judiciais...")
    ...
    def _scrape_category(self, cat_name, cat_path):
        for page in range(0, config.MAX_PAGES_PER_SOURCE):
            url = f"{self.base_url}{cat_path}?pagina={page}"
            try:
                resp = self._fetch(url)
                soup = self._parse_html(resp.text)
            except Exception:
                self.logger.info(f"cloudscraper blocked for {cat_name}, trying Selenium")
